...
const nftItems = [
  { id: "VRA-9901", name: "Transcendent Origin #01", tier: 12, price: "1,200", type: "Artifact" },
...
export function VeyraNFTDiscovery() {
  return (
    <div className="space-y-12">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
           <h3 className="text-3xl font-black tracking-tighter italic uppercase mb-2">Discovery Protocol</h3>
           <p className="text-slate-400 text-sm max-w-xl">A specialized marketplace for rare Veyra identity artifacts and Visual AI patterns.</p>
        </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {nftItems.map((nft) => (
          <div key={nft.id} className="bg-zinc-900 border border-white/10 rounded-3xl overflow-hidden group hover:border-indigo-500/50 transition-all shadow-2xl">
            {/* Visual Preview */}
            <div className="aspect-square bg-[#050505] p-12 flex items-center justify-center relative overflow-hidden">
               ...
               <VeyraBadge tier={nft.tier as any} size="lg" />
            ...
            <div className="p-8">
               <div className="flex items-center justify-between mb-4">
                  <div>
                    <h4 className="font-bold text-lg italic tracking-tight">{nft.name}</h4>
                    <p className="text-[10px] text-slate-500 font-mono">SERIAL: {nft.id}</p>
                  </div>
               ...
               <div className="flex items-center justify-between mt-8 pt-6 border-t border-white/5">
                  <div className="flex items-center gap-2">
                     <span className="text-xs font-black italic">{nft.price}</span>
