...
class BiometricMonitor:
    ...
    async def start_monitoring(self, device_type: str = "apple_watch"):
        ...
        self._device_interface = await self._connect_device(device_type)
        ...
        while self.is_monitoring:
            try:
                ...
                if reading:
                    ...
                    if self.baseline_hrv is None and len(self.readings) > 30:
                        self._establish_baselines()
                    
                    # Assess stress if we have enough data
                    if len(self.readings) >= 10:
                        assessment = self.assess_stress()
                        if assessment.current_level in [StressLevel.HIGH, StressLevel.CRITICAL]:
                            await self._alert_handlers(assessment)
                
                await asyncio.sleep(self.reading_interval)
            except Exception as e:
                logger.error(f"Biometric monitoring error: {e}")
...
def _simulate_variation(self) -> float:
    ...
    def assess_stress(self) -> StressAssessment:
        """Assess current stress level from recent readings."""
        if len(self.readings) < 10:
            return StressAssessment(
                current_level=StressLevel.NORMAL,
                overall_score=0.5,
                trend="stable",
                recommendation="Insufficient data - monitoring...",
                should_pause_trading=False,
                window_readings=list(self.readings)
            )
        
        recent = list(self.readings)
        
        # Calculate average metrics
        avg_hr = sum(r.heart_rate for r in recent) / len(recent)
        avg_hrv = sum(r.hrv_score for r in recent) / len(recent)
        avg_gsr = sum(r.gsr_level for r in recent) / len(recent)
        
        # Calculate stress components
        hr_stress = self._calculate_hr_stress(avg_hr)
        hrv_stress = self._calculate_hrv_stress(avg_hrv)
        gsr_stress = self._calculate_gsr_stress(avg_gsr)
        
        # Weighted overall score
        overall_score = (hr_stress * 0.35 + hrv_stress * 0.4 + gsr_stress * 0.25)
        ...
        recommendation = self._generate_recommendation(current_level, trend)
        ...
        return StressAssessment(
            ...
            should_pause_trading=current_level in [StressLevel.HIGH, StressLevel.CRITICAL],
            window_readings=recent
        )
    
    def _calculate_hr_stress(self, heart_rate: float) -> float:
        """Calculate stress from heart rate (0-1 scale)."""
        if self.baseline_hr:
            deviation = abs(heart_rate - self.baseline_hr) / self.baseline_hr
            return min(1.0, deviation * 2)
        # Default baseline 70 BPM
        return max(0, min(1.0, (heart_rate - 60) / 60))
    
    def _calculate_hrv_stress(self, hrv: float) -> float:
        """Calculate stress from HRV (lower HRV = more stress)."""
        if self.baseline_hrv:
            ratio = hrv / self.baseline_hrv
            return max(0, min(1.0, 1 - ratio))
        # Default baseline 50ms RMSSD
        return max(0, min(1.0, 1 - (hrv / 50)))
    
    def _calculate_gsr_stress(self, gsr: float) -> float:
        """Calculate stress from GSR (higher = more arousal)."""
        # Typical range 1-10 microsiemens
        return min(1.0, gsr / 8.0)
    ...
    def _calculate_trend(self) -> str:
        """Calculate stress trend from reading window."""
        if len(self.readings) < 20:
            return "stable"
        
        recent = list(self.readings)[-10:]
        earlier = list(self.readings)[:10]
        
        recent_stress = sum(r.stress_indicators["hr_stress"] for r in recent) / len(recent)
        earlier_stress = sum(r.stress_indicators["hr_stress"] for r in earlier) / len(earlier)
        
        diff = recent_stress - earlier_stress
        if diff < -0.1:
            return "improving"
        elif diff > 0.1:
            return "worsening"
    ...
    def _generate_recommendation(self, level: StressLevel, trend: str) -> str:
        """Generate personalized recommendation."""
        recommendations = {
            StressLevel.RELAXED: "Optimal state for complex decisions. Good time for strategic trades.",
            StressLevel.NORMAL: "Normal trading state. Proceed with standard risk management.",
            StressLevel.ELEVATED: "Take a short break. Consider reducing position sizes.",
            StressLevel.HIGH: "Pause trading. Do breathing exercises. Wait for stress to decrease.",
            StressLevel.CRITICAL: "STOP TRADING IMMEDIATELY. High risk of emotional decisions."
        }
        
        base = recommendations.get(level, "Monitor stress levels.")
        
        if trend == "worsening" and level.value in ["elevated", "high"]:
            base += " Stress is increasing - take preventive action now."
        
        return base
    
    async def _alert_handlers(self, assessment: StressAssessment):
        """Alert registered handlers."""
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        """Get summary of entire monitoring session."""
        ...
        for reading in readings_list:
            indicators = reading.stress_indicators
            score = (indicators["hr_stress"] * 0.35 + 
                    indicators["hrv_stress"] * 0.4 + 
                    indicators["gsr_stress"] * 0.25)
            scores.append(score)
        
        return {
            "duration_seconds": len(readings_list) * self.reading_interval,
            "average_stress_score": sum(scores) / len(scores),
            "max_stress_score": max(scores),
            "min_stress_score": min(scores),
            "high_stress_periods": sum(1 for s in scores if s > 0.6),
            "recommendation": "Good session" if sum(scores)/len(scores) < 0.4 else "High stress session - review trades"
        }


class StressBasedPositionSizer:
    """
    Adjusts position sizes based on real-time stress levels.
    """
    
    def __init__(self, monitor: BiometricMonitor):
        self.monitor = monitor
        self.base_position_size = 1.0
        
    def calculate_position_multiplier(self) -> float:
        """Calculate position size multiplier based on stress."""
        assessment = self.monitor.assess_stress()
        
        multipliers = {
            StressLevel.RELAXED: 1.2,  # Can take slightly more risk when calm
            StressLevel.NORMAL: 1.0,   # Standard position size
            StressLevel.ELEVATED: 0.7, # Reduce size
            StressLevel.HIGH: 0.4,     # Significant reduction
            StressLevel.CRITICAL: 0.0  # No trading
        }
        
        return multipliers.get(assessment.current_level, 0.5)
    
    def should_allow_trade(self) -> bool:
        """Check if current stress level permits trading."""
        assessment = self.monitor.assess_stress()
        return not assessment.should_pause_trading


# Example usage
async def example_usage():
    """Example of biometric monitoring."""
    monitor = BiometricMonitor()
    
    async def alert_handler(assessment: StressAssessment):
        print(f"\n⚠️ STRESS ALERT!")
        print(f"Level: {assessment.current_level.value}")
        print(f"Score: {assessment.overall_score:.2f}")
        print(f"Recommendation: {assessment.recommendation}")
    
    monitor.on_stress_alert(alert_handler)
    ...
    print(f"\nSession Summary:")
    print(f"Average Stress: {summary['average_stress_score']:.2f}")
    print(f"Max Stress: {summary['max_stress_score']:.2f}")
